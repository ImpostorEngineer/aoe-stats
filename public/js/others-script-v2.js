...
async function getMapData() {
  ...
  const mapData = await fetch(map_url, { mode: 'cors' })
    .then((response) => response.json())
    .then((mapdata) => {
      return mapdata;
    })
...
async function getUserRank(userid) {
  const rankOneVOneURL =
    'https://aoe2.net/api/player/ratinghistory?game=aoe2de&leaderboard_id=3&count=1&profile_id=' + userid;
  const rankOneVOne = await fetch(rankOneVOneURL, { mode: 'cors' })
    .then((response) => response.json())
    .then((pOnedata) => {
      return pOnedata;
    })
  ...
  const rankTGURL =
    'https://aoe2.net/api/player/ratinghistory?game=aoe2de&leaderboard_id=4&count=1&profile_id=' + userid;
  const rankTG = await fetch(rankTGURL, { mode: 'cors' })
    .then((response) => response.json())
    .then((pOnedata) => {
      return pOnedata;
    })
  ...
  return { rankOneVOne, rankTG };
}
const test = getUserRank(1885522).then((response) => console.log(response.rankOneVOne[0]));
